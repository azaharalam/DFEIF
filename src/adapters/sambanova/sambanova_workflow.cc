...
std::string SambaNovaWorkflow::OutputDir(const dfabit::api::Context& ctx) {
  return (std::filesystem::path(ctx.run_context().config().output.base_output_dir) /
          "platform" / "sambanova")
      .string();
}

std::string SambaNovaWorkflow::DetectPath(
...
void SambaNovaWorkflow::AppendProcessMetrics(
...
dfabit::core::Status SambaNovaWorkflow::MaybeRunCompile(
  ...
  const auto cmd = DetectPath(*ctx, "sambanova_compile_cmd");
  if (cmd.empty()) {
    return dfabit::core::Status::Ok();
  }

  const auto work_dir = DetectPath(*ctx, "sambanova_work_dir");
  const auto out_dir = OutputDir(*ctx);
  std::filesystem::create_directories(out_dir);

  dfabit::platform::ProcessSpec spec;
  spec.name = "sambanova_compile";
  spec.command = cmd;
...
dfabit::core::Status SambaNovaWorkflow::MaybeRunExecution(
    dfabit::api::Context* ctx,
  ...
  const auto cmd = DetectPath(*ctx, "sambanova_run_cmd");
  if (cmd.empty()) {
    return dfabit::core::Status::Ok();
  }
